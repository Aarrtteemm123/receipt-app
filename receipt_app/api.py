...
class ReceiptApi:
    ...
    @login_required
    async def create_receipt(self, request: Request, body: ReceiptRequest) -> ReceiptResponse:
        user = request.state.user
        total = sum(p.price * p.quantity for p in body.products)
        rest = max(body.payment.amount - total, 0)

        if body.payment.amount < total:
        ...
        payment_type = await PaymentType.get(name=body.payment.type)
        ...
        return ReceiptResponse(
            ...
            payment=PaymentOutput(**body.payment.model_dump()),
            total=round(total, 2),
            rest=round(rest, 2),
